...
object TranslateGene extends GeneGenerator {
  override def forProblem(problem: Problem): Seq[Gene] = {
    ...
    val xMove = Random.nextInt(xDiff) * Random.nextDouble() * (if (Random.nextBoolean()) 1d else 0)
    val yMove = Random.nextInt(yDiff) * Random.nextDouble() * (if (Random.nextBoolean()) 1d else 0)
